...
class ViewController: UIViewController {
    ...
    func selfSignedSSLSetting() {
        let clientID = "CocoaMQTT-IOS-DemoMQTTProtobuf"
        ...
        let clientCertArray = getClientCertFromP12File(certName: "cert_key", certPassword: "emq123")
    ...
    func parseControllerDetails(responseString: String) {
        ...
        let response = decodeProtoBufDeviceMQTT(response: responseString)
        print(response)
        print("response origin \(response.origin)")
        print("response identifier \(response.identifier)")
        print(response.data!)
        if let data = response.data.data(using: .utf8) {
            do {
                let dictonary = (try JSONSerialization.jsonObject(with: data, options: []) as? [[String:AnyObject]])!
                print(dictonary)
                let arrData = dictonary as NSArray
                print(arrData)
                print(arrData.count)
            } catch {
    ...
    func decodeProtoBufDeviceMQTT(response: String) -> ControllerData {
...
extension ViewController: CocoaMQTTDelegate {
    ...
    func mqtt(_ mqtt: CocoaMQTT, didConnectAck ack: CocoaMQTTConnAck) {
        ...
        if ack == .accept {
            print("connect ack")
            let subTopic = "epms/controllerdata/5afbfa7b11c6c63e53e8c93a"
...
extension String {
    var hexa2Bytes: [UInt8] {
        ...
        return stride(from: 0, to: characters.count, by: 2).compactMap { UInt8(String(hexa[$0..<$0.advanced(by: 2)]), radix: 16) }
    }
}
